...
2. **Semi-supervised anomaly detection by variational-autoencoder (VAE)**. In this project, the VAE model learned in the training phase only normal samples to create a "normal model". To find anomalies in the test data, we used the error function of the VAE model as a measure of how much the test samples are far from the "normal model". To get a function that will give the best separation for finding anomalies, we examined a different weight for the parameters representing the two error components.
